...
$comando = "update `$linha[1]`.`clientes` set `codigo` = '$cod_cliente',`telemarketing` = 'Em Manutenção',`operador_manutencao` = '$operador',`cidade_estab_pertence_manutencao` = '$cidade_estab_pertence',`dia_liberado` = '',`mes_liberado` = '',`ano_liberado` = '',`hora_liberado` = '' where `clientes`. `codigo` = '$cod_cliente' limit 1 ";
}
...
if($quantidade==0){
$comando = "insert into telemarketing(dia_abertura,mes_abertura,ano_abertura,hora_abertura,status,operador,estab_pertence,cidade_estab_pertence,cod_cliente,cpf,nome,telefone,celular) 
values('$dia_abertura','$mes_abertura','$ano_abertura','$hora_abertura','$status','$operador','$estab_pertence','$cidade_estab_pertence','$cod_cliente','$cpf','$nome','$telefone','$celular')";
