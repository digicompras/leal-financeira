...
$mesontem = date('m');
$anoontem = date('Y');
$dataontem = mktime(0,0,0,$mesontem,$diaontem,$anoontem);
$ontem = date('Y-m-d',$dataontem);
$dataanterior = "$diaontem-$mesontem-$anoontem";
...
<?php
	...
	$ultimo_registro_de_ponto = strtotime($ultimo_registro_de_ponto);// data do vencimento BDados
	$diferenca = $ultimo_registro_de_ponto - $hoje;//calculando a diferença
	$dias = (int)floor( $diferenca / (60 * 60 * 24))*-1;// dando a diferença em dias
...
$num_dia = date('w');
$num_dia_anterior = bcsub($num_dia,1);

$sql = "SELECT * FROM dias_semana where num_dia = '$num_dia_anterior' limit 1";

$res = mysql_query($sql);

while($linha=mysql_fetch_row($res)) {

$dia_semana_anterior = $linha[2];

}



$comando = "insert into ponto(nome,data,ent_m,mes_ano,sai_m,ent_t,sai_t,dia_semana,dia,mes,ano,date)";
if($dia_semana_anterior<>"Domingo"){
echo "values('$nome','$dataanterior','FALTOU','$mes_ano','FALTOU','FALTOU','FALTOU','$dia_semana_anterior','$diaontem','$mesontem','$anoontem','$ontem')";
}
else{
echo "values('$nome','$dataanterior','DSR','$mes_ano','DSR','DSR','DSR','$dia_semana_anterior','$diaontem','$mesontem','$anoontem','$ontem')";
}
