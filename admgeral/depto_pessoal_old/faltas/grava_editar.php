...
$comando = "update `$linha[1]`.`faltas` set `codigo` = '$codigo',`data` = '$data',`dia` = '$dia',`mes` = '$mes',`ano` = '$ano',`hora_inicio` = '$hora_inicio',`hora_termino` = '$hora_termino',`hi` = '$hi',`mi` = '$mi',`ht` = '$ht',`mt` = '$mt',`nome` = '$nome',`quant_horas_reais` = '$quant_horas_reais',`quant_horas` = '$quant_horas_decimais',`valor_hora_normal` = '$valor_hora_normal',`acrescimo` = '$acrecimo_hora_extra',`valor_hora_extra` = '$valor_hora_extra',`total` = '$total',`salario` = '$salario' where `faltas`. `codigo` = '$codigo' limit 1 ";

}



mysql_query($comando,$conexao) or die("Erro ao alterar informações do registro de hora falta desse operador");
